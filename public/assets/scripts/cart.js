...
addToCartButtons.forEach(function (button) {
    button.addEventListener("click", function (event) {
        ...
        var existingProduct = localStorage.getItem("cartProduct_" + productId);
        if (existingProduct) {
            showNotification("Este produto já está no carrinho!", "#e74c3c");
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                showNotification("Produto adicionado ao carrinho!", "#2ecc71");
                renderProductInCart(productId, productDetails);
            },
            error: function (xhr, status, error) {
                console.error(error);
...
function renderProductInCart(productId, productDetails) {
    var productData = JSON.parse(
        localStorage.getItem("cartProduct_" + productId)
    );
    ...
    deleteButton.addEventListener("click", function () {
        ...
        updateTotalItems();
        calculateTotalPrice();
        showNotification("Produto removido do carrinho", "#e1cd33");
    });
